defmodule LawAdvisorRyanTestWeb.CoreComponents.Button do
  @moduledoc """
  ...
  attr :link_type, :string, values: ["button", "a", "live_patch", "live_redirect"]
  attr :label, :string
  attr :size, :string
  attr :variant, :string
  attr :to, :string
  ...
  def button(assigns) do
    ...
    ~H"""
    <CC.a to={@to} link_type={@link_type} class={@classes} disabled={@disabled} {@rest}>
      <%= if @loading do %>
        <CC.spinner show={true} size_class={get_spinner_size_classes(@size)} />
      <% end %>
  ...
  def icon_button(assigns) do
    ...
    >
      <%= if @loading do %>
        <CC.spinner show={true} size_class={get_icon_button_spinner_size_classes(@size)} />
      <% else %>
        <%= render_slot(@inner_block) %>
      <% end %>
    </CC.a>
